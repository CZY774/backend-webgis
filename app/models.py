...
class UMKM(Base):
    ...
    nama = Column(String(255), nullable=False)
    jenis = Column(String(100), nullable=False, index=True)
    pemilik = Column(String(255), nullable=True)
    lokasi = Column(Text, nullable=True)
...
class Wisata(Base):
    ...
    jenis = Column(String(100), nullable=False, index=True)
    deskripsi = Column(Text, nullable=True)
    cagar_budaya = Column(Text, nullable=True)
    lokasi = Column(Text, nullable=True)
    tarif = Column(String(100), nullable=True)
...
class KependudukanRT(Base):
    __tablename__ = "kependudukan_rt"
    __table_args__ = {"extend_existing": True}
    id_kependudukan_rt = Column(Integer, primary_key=True, index=True)
    id_rt = Column(
        Integer, ForeignKey("rt.id_rt", ondelete="CASCADE"), unique=True, nullable=False
    )
    jumlah_kk = Column(Integer, nullable=True)
    jumlah_warga = Column(Integer, nullable=False)
    laki_laki = Column(Integer, nullable=False)
